...
어려웠던점(배운점):
1. 문제 아이디어 자체는 직관적이어서 어렵지 않았지만, 시간복잡도를 고려해서 풀어야 한다는 것을 분명하게 깨달은 문제였다.
또한 투포인터를 쓰는 이유를 확실하게 깨달을 수 있었다. stack구조를 실제로 변형시키면 (특히 pop이 아니라 중간요소를 변형하면) O(n)이 발생하기 때문. 
2. stack에는 delete() 문법이 없으므로 특정값을 삭제하고 싶다면 remove를 쓸것 
3.reversed(people) 문법 
